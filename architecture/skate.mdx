...
<Note>
  A standard flow through Skate:
  <ol>
    <li>
    Intent creation: Users initiate intents on different originating chains (EVM, TON, Solana, ...). These intents specify the desired actions or outcomes.
    </li>
    <li>
    Intent Execution: Executors pick up these intents and execute the necessary logic on the kernel contract deployed on the Nollie chain. This kernel manages the core state and logic of the application.
    </li>
    <li>
    Preconfirmation AVS: If the execution requires interaction or settlement between the kernel and periphery components on different chains, the Preconfirmation AVS validates these interactions.
    </li>
    <li>
    Settlement: Once validated by the Preconfirmation AVS, the chosen executor carry out relevant interactions between the kernel and the respective periphery contracts on the destination chains.
    </li>
  </ol>
</Note>
